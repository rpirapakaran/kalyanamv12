...
class Design
{
    ...
    {
        echo '<strong>', t('Proudly Powered by'), ' <a href="', Kernel::SOFTWARE_WEBSITE, '" title="', Kernel::SOFTWARE_DESCRIPTION, '">', Kernel::SOFTWARE_NAME, '</a> ', Kernel::SOFTWARE_VERSION, '</strong>';
    }
    ...
    public function getUserAvatar($sUsername, $sSex = '', $iSize = null, $bPrint = true)
    {
        ...
        if (!$sUrl = $oCache->get()) {
            ...
            if (!is_file($sPath) || $oGetAvatar->approvedAvatar == '0') {
                ...
                $sIcon = ($sSex === 'male' || $sSex === 'female' || $sSex === PH7_ADMIN_USERNAME) ? $sSex : 'visitor';
